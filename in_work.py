...
viber = Api(BotConfiguration(
    name='createPDF',
    avatar='',
    auth_token='47d949c38ba7d354-26d746d90c8ed71-77df6d1bf9cfaa64'
))
        ...
        if row['Title'] and row['Description']:
            ...
            if art in row['SKU'].strip():
                if 'Артикул' in row['Title']:
                    ...
                    break
                elif row['Title'].strip().split(' ')[1] == art:
                    art_if = row['Title'].strip().split(' ')[0] + " " + row['SKU'].strip()
                    price = row['Price'].strip()
                    link = row['Photo'].strip()
                    break
...
@application.route('/47d949c38ba7d354-26d746d90c8ed71-77df6d1bf9cfaa64/', methods=['POST'])
def incoming():
    global para
    global rows
    global c_ind
    ...
    if isinstance(viber_request, ViberMessageRequest):
        if viber_request.message.text == 'Настройки параметров':
            ...
            with open('for_pdf.csv', encoding='utf-8') as f:
                ...
                if viber_request.message.text == 'Все в наличии':
                    ...
                    uchar = udata['char']
                    if ulink:
                        viber.send_messages(viber_request.sender.id, [PictureMessage(media=ulink, keyboard={
                            "DefaultHeight": True,
                            "BgColor": "#FFFFFF",
                            "Type": "keyboard",
                            "Buttons": [get_buttons('Артикул и Размеры'),
        ...
        viber.send_messages(viber_request.user.id, [
            TextMessage(text=f"Здравствуйте!\n Нажмите\n'Настройки параметров'",
                        keyboard=keyboard)
        ])
        # message_timestamp = viber_request.timestamp
        # viber.send_messages(viber_request.user.id, [
        #     TextMessage(text=f"{message_timestamp}", keyboard=keyboard)
        # ])
